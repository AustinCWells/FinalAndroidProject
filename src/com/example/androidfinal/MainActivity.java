...
public class MainActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		allButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// replace all items in list view 
	...
	private void loadOptions() {
		//  run fragment to display study location preferences 
	...
	// After item is select, format with delemeter and send to filter 
	public void onOptionsSelected(Boolean chain, Boolean wifi, Boolean events,
			Boolean coffee, Boolean food, Integer cost, String atmosphere,
			Integer rating) {
		String constraints = chain.toString() + "|" + wifi.toString() + "|"
				+ events.toString() + "|" + food.toString() + "|"
				+ cost.toString() + "|" + atmosphere.toString() + "|"
				+ rating.toString();
	...
	private class loadJobsTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... urls) {
			...
			try {
				...
				for (int i = 0; i < locationsArray.length(); i++) {
					...
					ExpListItems.add(new ExpandListGroup(new Location(name,
							address, phone, weekday_hours, weekend_hours,
							rating, price, hasCoffee, hasFood, isChain,
							hasWifi, hasEvents, atmosphere)));
		...
		protected void onPostExecute(String loginResult) {
			// remove progress bar and load expandable list adapter 
	...
	// Location class used to study data for each study location
	public class Location {
		...
		public Location(String name, String address, String phone,
				String weekday_hours, String weekend_hours, Integer rating,
				Integer price, Boolean hasCoffee, Boolean hasFood,
				Boolean isChain, Boolean hasWifi, Boolean hasEvents,
				String atmosphere) {
			this.name = name;
	...
	public class ExpandListAdapter extends BaseExpandableListAdapter implements
		...
		public View getChildView(int groupPosition, int childPosition,
			...
			if (view == null) {
				...
				view = infalInflater.inflate(R.layout.expandlist_child_item,
						null);
			}
			// set study locations data 
			TextView addressView = (TextView) view.findViewById(R.id.address);
			TextView phoneView = (TextView) view.findViewById(R.id.phone);
			TextView weekdayHoursView = (TextView) view
					.findViewById(R.id.weekday_hours);
			...
			// replace rating number with dollar signs 
			...
			// when address is clicked go to external map application
			...
			// when phone number is clicked go to external phone app 
		...
		public View getGroupView(int groupPosition, boolean isLastChild,
			...
			TextView groupView = (TextView) view
					.findViewById(R.id.studyLocationGroup);
		...
		// Filter data based on the passed in constraints 
		@Override
		public Filter getFilter() {
			...
			Filter filter = new Filter() {
				...
				protected FilterResults performFiltering(CharSequence constraint) {
					groups = ExpListItems;
					// if constrain is all replace all locations in list view 
					...
					// check if cost should be ignored 
					if (cost == 0) {
						cost = 5;
					} else {
						cost -= 1;
					...
					// check if rating should be ignored 
					if (rating == 1) {
						rating = 0;
					} else {
						rating -= 1;
					...
					//remove unwanted restraunts 
					for (int i = 0; i < groups.size(); i++) {
						ExpandListGroup groupItem = groups.get(i);
						Location location = groupItem.getLocation();
						// check all boolean locations 
						if (location.isChain == isChain
								&& location.hasWifi == hasWifi
								&& location.hasEvents == hasEvents
								&& location.hasFood == hasFood) {

							
							
							// check if atmosphere should be ignored 
							if (atmosphere.contains("no preference")) {
								Log.i("ACW", "price: " + location.price + "cost: " + cost);
								if (location.price <= cost
